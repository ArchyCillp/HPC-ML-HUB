...
var GHSyncPlugin = class extends import_obsidian.Plugin {
  async SyncNotes() {
    ...
    function getStatusSummary(status) {
        return [
            ...status.not_added.map(f => `新增未跟踪: ${f}`),
            ...status.created.map(f => `新增文件: ${f}`),
            ...status.modified.map(f => `修改文件: ${f}`),
            ...status.deleted.map(f => `删除文件: ${f}`),
            ...status.renamed.map(f => `重命名: ${f.from} → ${f.to}`)
        ].join('\n');
    }

    async function generateCommitMessage(statusSummary) {
        const prompt = `基于以下Git变更生成一个简洁的提交信息，要求：
    1. 使用中文
    2. 不超过30个汉字
    3. 只返回提交信息本身！！！！一定只返回commit信息本身！否则我这边解析不了。比如只返回：“feat: 新增xxxx”
    4. 符合Conventional Commits规范
    5. 优先体现主要变更类型
    6. 因为本项目基本上全都是文档，所以如果改的文档比较少，可以在提交信息里概括一下改了什么文档，但还是不超过30个汉字
    7. 如果修改了很多，请把重点放在最有趣的部分（比如新增了新的文章），其他的部分可以简略开过，但还是不超过30个汉字

    变更列表：
    ${statusSummary}`;

        try {
            new import_obsidian.Notice('deepseek生成提交信息成功');
            const response = await fetch('https://api.siliconflow.cn/v1/chat/completions', {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer <deepseek的密钥>',
            ...
            new import_obsidian.Notice('deepseek生成提交信息失败，使用默认格式', 1e4);
            return null;
        }
    }

    // 生成智能提交信息或回退到默认格式
    const statusSummary = getStatusSummary(statusResult);
    const aiCommitMsg = statusSummary ? await generateCommitMessage(statusSummary) : null;

    // 最终使用的提交信息
    let msg = aiCommitMsg || `${hostname} ${new Date().toISOString().slice(0, 19).replace('T', ' ')}`;
